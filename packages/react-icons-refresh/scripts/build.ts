...
async function main() {
  try {
    // @react-icons-refresh/all
    const allOpt: TaskContext = {
      rootDir: _rootDir,
      DIST: path.resolve(_rootDir, "../_react-icons-refresh_all"),
      LIB: path.resolve(_rootDir, "../_react-icons-refresh_all/lib"),
    };
    await task("@react-icons-refresh/all initialize", async () => {
      ...
      await taskCommon.writePackageJson(
        { name: "react-icons-refresh", exports: buildPackageExports(icons) },
        allOpt,
      );
      await taskCommon.copyReadme(allOpt);
    });
    await task("@react-icons-refresh/all write icons", async () => {
      await Promise.all(
        icons.map((icon) => taskAll.writeIconModule(icon, allOpt)),
      );
    });

    // @react-icons-refresh/all-files
    const filesOpt: TaskContext = {
      rootDir: _rootDir,
      DIST: path.resolve(_rootDir, "../_react-icons-refresh_all-files"),
      LIB: path.resolve(_rootDir, "../_react-icons-refresh_all-files/lib"),
    };
    await task("@react-icons-refresh/all-files initialize", async () => {
      ...
      await taskCommon.writePackageJson(
        { name: "@react-icons-refresh/all-files" },
        filesOpt,
      );
      await taskCommon.copyReadme(filesOpt);
    });
    await task("@react-icons-refresh/all-files write icons", async () => {
      await Promise.all(
    ...
    await task("react-icons-refresh_builders write icon versions", async () => {
      await taskCommon.writeIconVersions(filesOpt);
    });

    // write to d.ts files
    await task(
      "react-icons-refresh_builders build common library",
      async () => {
